...
public class MouseGame extends Game implements Scene {
    ...
    // Handy reference for center of the screen
    XYPair<Integer> center;
    ...
    public MouseGame()
    {
        ...
        center  = new XYPair<>((ui.getWidth())/2, ui.getHeight()/2);

        bullets = new java.util.LinkedList<Bullet>();
        player = new Turret();
        enemies = new java.util.LinkedList<Enemy>();
        collectibles = new java.util.LinkedList<Collectible>();
    ...
    {
        private int size = ui.getWidth()/10;
        {
            this.hitbox.setBounds(center.x-size/2, center.y-size/2, size, size);
            this.r = 0f;
            this.g = 0.6f;
            this.b = 0f;
        }
    ...
    private class Bullet extends GameObject
    {
        ...
        {
            // Make the bullet
            this.hitbox.setBounds(center.x, center.y, 5, 5);
            // Use destination point to calculate x and y components of the direction vector using trigonometry
            XYPair<Float> clamped = new XYPair<>((float)(destination.x - center.x)*2 / ui.getWidth(),2*(float)(destination.y - center.y)/ui.getHeight());
        ...
        public void update (int delta)
        {
            ...
            if ((this.hitbox.getX()+this.hitbox.getWidth())< 0 || this.hitbox.getX() > ui.getWidth() ||
                    this.hitbox.getY() > ui.getHeight() || (this.hitbox.getY() + this.hitbox.getHeight() < 0))
            {
                this.deactivate();
            }
        }
    ...
    private class Enemy extends GameObject
    {
...

        {
            ...
            this.size = (3 * maxHealth - 20) * ui.getWidth() / 800;
            ...
            do {
                ...
                int x = center.x + (int) (center.x * Math.cos(Math.toRadians(posAngle)) - size / 2);
                int y = center.y - (int) (center.y * Math.sin(Math.toRadians(posAngle)) + size / 2);

                //  Make sure the Enemy is within the window
                if (x < 0) {    x = 0; }
                if (x + size > center.x * 2) {  x = center.x * 2 - size;    }
                if (y < 0) {    y = 0;  }
                if (y + size > ui.getHeight()) {    y = ui.getHeight() - size;  }
            ...
            XYPair<Float> clamped = new XYPair<>((float)(this.hitbox.x + size/2 - center.x) / center.x,(float)(this.hitbox.y + size/2 - center.y)/center.y);
        ...
        public void update(int delta) {
            ...
            if  (   this.hitbox.x + size < 0 || this.hitbox.x > center.x*2 ||
                    this.hitbox.y + size < 0 || this.hitbox.y > ui.getHeight()  )
            {   this.deactivate(); }
        }
    }
    ...
    private class Collectible extends GameObject
    {
        ...
        public Collectible (int x, int y)
        {
            ...
            int size = (int) (35 * ui.getWidth() / 800.);
        ...
        public void update(int delta) {
            ...
            if (captured) {
                ...
                XYPair<Integer> mousePos = ui.getMouseLocation();
    ...
    public Scene drawFrame(int delta)
    {
        ...
        if (ui.mouseButtonIsPressed(0))
        {
            XYPair<Integer> mouseLoc = ui.getMouseLocation();
